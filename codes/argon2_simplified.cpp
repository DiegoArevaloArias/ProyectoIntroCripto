...
static std::vector<u8> blake2b_vec(const std::vector<u8> &in, size_t outlen) {
  ...
  out = blake2b(out, outlen, in, in.size());
  return out;
}
...
&salt, const Argon2Params &p) {
  ...
  auto push32 = [&](uint32_t x){
    for (int i=0;i<4;++i)
      buf.push_back((u8)((x>>(8*i))&0xFF));
  };

  push32(p.time_cost);
  push32(p.mem_kib);
  push32(p.parallelism);
  push32(p.tag_len);
...
std::vector<uint8_t> argon2_hash(const std::string &password, const
  ...
  for (uint32_t pass = 0; pass < params.time_cost; ++pass) {
    ...
    for (size_t idx = 0; idx < q; ++idx) {
        ...
        const Block &X = (idx == 0) ? lanes[lane][q-1] : lanes[lane][idx-1];
        ...
        if (pass == 0 && idx < q/2) {
          ...
          for (int i=0;i<4;++i) seed.push_back((u8)((pass >> (8*i)) & 0xFF));
          for (int i=0;i<4;++i) seed.push_back((u8)((lane >> (8*i)) & 0xFF));
          for (int i=0;i<4;++i) seed.push_back((u8)((idx >> (8*i)) & 0xFF));
