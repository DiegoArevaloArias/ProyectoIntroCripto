...
void compress(u64 h[8], const u8 block[128], uint64_t t_low, uint64_t t_high, bool last) {
  u64 v[16];
  u64 m[16];
  for (int i = 0; i < 16; ++i) {
  uint64_t w = 0;
  for (int j = 0; j < 8; ++j) w |= (uint64_t)block[i*8 + j] << (8*j);
  m[i] = w;
  }
...
const unsigned char *salt, const unsigned char *person) {
  ...
  if (salt) memcpy(param + 32, salt, 16);
  if (person) memcpy(param + 48, person, 16);

  u64 h[8];
  for (int i = 0; i < 8; ++i) h[i] = IV[i];
  for (int i = 0; i < 8; ++i) {
    uint64_t w = 0;
    for (int j = 0; j < 8; ++j) w |= (uint64_t)param[i*8 + j] << (8*j);
    h[i] ^= w;
  }
  size_t offset = 0;
  ...
  memset(block, 0, 128);
  if (left) memcpy(block, in + offset, left);
